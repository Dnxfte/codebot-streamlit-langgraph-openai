...
Нижче наведено реальні приклади роботи застосунку. Вони показують основні сценарії CodeBot: звичайний чат, LangGraph-інструменти, роботу зі snippets, швидкі дії у вкладці коду, Wikipedia/calculator та статистику сесії.

### 1. Звичайний чат: пояснення Python-коду

На скріншоті користувач просить пояснити код:
...
CodeBot пояснює створення списку, роботу list comprehension, виклик `print`, очікуваний результат `[1, 4, 9, 16]`, а також коротко описує потенційні ризики та можливі оптимізації.

![Звичайний чат із поясненням Python-коду](screens/chat-explain.png)

### 2. LangGraph-агент: статичний аналіз і журнал інструментів

На скріншоті показано приклад ревʼю небезпечного Python-коду з hardcoded password, `eval(input(...))` та широким `except Exception`. Агент відповідає у структурі Code Reviewer: що робить код, покрокове пояснення, потенційні проблеми, рекомендації та кращий варіант.

У нижній частині видно `Журнал інструментів агента`, де зафіксовано виклик `analyze_code_static` і результат базового статичного аналізу.

![LangGraph-агент зі статичним аналізом](screens/agent-static-analysis.png)

### 3. LangGraph-агент: збереження та перегляд snippets

На скріншоті користувач просить зберегти код як `"приклад сортування"`, після чого агент показує список збережених фрагментів і повний код snippet за номером.

Демонструються інструменти `save_snippet`, `list_snippets`, `show_snippet`, а також сценарій роботи з колекцією snippets без ручного копіювання коду.

![Збереження та перегляд snippets](screens/agent-snippets.png)

### 4. Вкладка "Код": швидка оптимізація

На скріншоті показано вкладку `📝 Код`: ліворуч користувач вставляє код у `st.text_area`, бачить підсвітку через `st.code` і використовує швидку дію `🔧 Оптимізувати`. Праворуч CodeBot показує результат аналізу та пропонує компактніший варіант через list comprehension:

```python
result = [item * 2 for item in items if item > 10]
```

![Швидка оптимізація у вкладці Код](screens/code-optimization.png)

### 5. Wikipedia та calculator у режимі агента

На скріншоті користувач просить знайти коротку інформацію про `list comprehension` у Wikipedia і порахувати вираз `2 * (8 + 4)`. Агент використовує два інструменти: `wikipedia_search` і `calculator`.

У журналі видно обидва виклики інструментів та результати: довідку з Wikipedia і обчислення `2 * (8 + 4) = 24`.

![Wikipedia та calculator у LangGraph-режимі](screens/agent-wikipedia-calculator.png)

### 6. Статистика сесії

На скріншоті показано вкладку `📊 Статистика`: кількість повідомлень, snippets, викликів агента, приблизну кількість символів у чаті, індикатор активності сесії, діаграму snippets за мовами та кнопку експорту повної сесії у JSON.

Цей екран демонструє компоненти `st.metric`, `st.progress`, `st.bar_chart`, `st.dataframe` і `st.download_button`.

![Статистика сесії CodeBot](screens/statistics.png)
